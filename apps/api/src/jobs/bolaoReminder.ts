...
export async function sendBolaoClosingReminders(pool: Pool): Promise<{ notified: number }> {
  // Membro do grupo, mercado ainda OPEN, fecha dentro da janela, sem palpite
  // dele ainda. Dedup por (user_id, market_id) reaproveitando a coluna que já
  // existe em notifications — simplificação aceita: dois bolões (tipos
  // diferentes) no mesmo mercado no mesmo grupo só disparam 1 lembrete.
  const r = await pool.query(
    `SELECT b.market_id, b.group_id, gm.user_id, u.email, u.email_notifications,
            m.title AS market_title, g.name AS group_name
       FROM boloes b
      ...
      WHERE m.status = 'OPEN'
        AND m.close_at BETWEEN now() AND now() + interval '${REMINDER_WINDOW_HOURS} hours'
        AND bp.user_id IS NULL
        AND NOT EXISTS (
          SELECT 1 FROM notifications n
           WHERE n.user_id = gm.user_id AND n.kind = 'BOLAO_CLOSING_SOON' AND n.market_id = b.market_id
        )`,
  );
  ...
  for (const row of r.rows) {
    ...
    await notify(pool, row.user_id, "BOLAO_CLOSING_SOON", body, {
      marketId: row.market_id, groupId: row.group_id,
    });
